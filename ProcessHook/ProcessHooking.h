...
public:
	...
	static void MakeProcessJMP(ProcessMemory* m_pRemoteSourceAddress, void* m_pRemoteDestinationAddress);

	// Makes a CALL hook
	static void MakeProcessCALL(ProcessMemory* m_pRemoteSourceAddress, ProcessMemory* m_pRemoteDestinationAddress);
	static void MakeProcessCALL(ProcessMemory* m_pRemoteSourceAddress, void* m_pRemoteDestinationAddress);
	...
	static inline void MakeMultipleProcessJMPs(std::initializer_list<ProcessMemory*> m_pRemoteSourceAddressList, void* m_pRemoteDestinationAddress)
	{
		for (auto m_nRemoteSrcAddr : m_pRemoteSourceAddressList)
		{
			MakeProcessJMP(m_nRemoteSrcAddr, m_pRemoteDestinationAddress);
	...
	static inline void MakeMultipleProcessCALLs(std::initializer_list<ProcessMemory*> m_pRemoteSourceAddressList, void* m_pRemoteDestinationAddress)
	{
		for (auto m_nRemoteSrcAddr : m_pRemoteSourceAddressList)
		{
			MakeProcessCALL(m_nRemoteSrcAddr, m_pRemoteDestinationAddress);
...
private:
	...
	static inline uint32_t GetRelativeProcessAddress(uint32_t src, void* dest)
	{
	...
	static inline uint32_t GetRelativeProcessAddress(ProcessMemory* m_pRemoteSourceAddress, void* m_pRemoteDestinationAddress)
	{
		return reinterpret_cast<uint32_t>(m_pRemoteDestinationAddress) - m_pRemoteSourceAddress->GetAddress();
	}
};